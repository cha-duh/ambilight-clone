...
public class proc_20SectionAmbilightCLEAN extends PApplet {
...
public void calculateTop() {
  for (int k=0; k<8; k++) {
    for (int i=k*displayWidth/8; i<(k+1)*displayWidth/8; i=i+2) {
...
public void calculateLeft() {
  ...
  for (int i = 0; i<displayWidth/8; i = i+pixelSpread) {
    for (int j = 2*displayHeight/4; j<3*displayHeight/4; j= j+pixelSpread) {
      pixel = screenshot.getRGB(i, j); //the ARGB integer has the colors of pixel (i,j)
      r0 = r0+(int)(MAX&(pixel>>16)); //add up reds
      g0 = g0+(int)(MAX&(pixel>>8)); //add up greens
...
public void stop() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "proc_20SectionAmbilightCLEAN" };
    if (passedArgs != null) {
